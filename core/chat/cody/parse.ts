/**
 * Parse Sourcegraph Cody CLI conversation JSON files into the normalized chat model.
 *
 * Cody stores each conversation at:
...
/**
 * Parse the raw JSON content of a Cody conversation file. Returns the parsed
 * object or null if it does not look like a valid session file.
 */
async function readSessionFile(
  filePath: string,
): Promise<CodySessionFile | null> {
  let raw: string
  try {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      return parsed as CodySessionFile
    }
  } catch {
...
export async function readCodyMeta(
  ref: ChatSessionFileRef,
): Promise<ChatSessionMeta | null> {
  const { filePath } = ref
  const id = codySessionId(filePath)
  ...
  const title = asString(data.title) || derivedTitle || 'Cody conversation'
...
export async function listCodySessions(
  env: OsEnv,
  opts?: ChatListOptions,
): Promise<ChatSessionPage> {
  const files = await listCodySessionFiles(env)
  return paginateByMtime(
    files.map((f) => ({ filePath: f, ref: { filePath: f } })),
    opts,
    (ref) => readCodyMeta(ref),
  )
}

export async function readCodySession(
  env: OsEnv,
  sessionId: string,
): Promise<ChatTranscript> {
  const filePath = await findCodySessionFile(env, sessionId)
...
/**
 * Delete a Cody session by removing its JSON file.
 * Wraps EACCES/EPERM in ConfigWriteError so the renderer can surface a
 * permission-denied toast.
 */
export async function deleteCodySession(
  env: OsEnv,
  sessionId: string,
): Promise<void> {
  const filePath = await findCodySessionFile(env, sessionId)
...
export async function listCodySessionFileRefs(
  env: OsEnv,
